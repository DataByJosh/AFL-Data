...
get_match_chains <- function(season = year(Sys.Date()), round = NA) {
  ...
  chains <- chains %>% select(matchId,season,roundNumber,utcStartTime,homeTeam.teamName:date,venue.name:venue.state,
...
get_round_games <- function(season,round) {
  ...
  if (length(games) > 0) {
    ...
    if (nrow(games) > 0) {
      games <- games %>% select(matchId,utcStartTime,roundNumber,venue.name,venue.location,venue.state,
